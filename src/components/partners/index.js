import React, { Component } from 'react';
import styled from 'styled-components';

export default class Partners extends Component {
  componentWillReceiveProps(nextProps) {
    if (nextProps.anchor && nextProps.anchor === 'participants') {
      const target = document.querySelector('#participants');
      target.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }

  render() {
    return (
      <S id="participants">
        <C>
          <H>Marketplace Participants</H>
          <Ul>
            <Li>
              <I src="/static/logotypes/42.png" srcSet="/static/logotypes/42.png 2x" alt="42" />
            </Li>
            <Li>
              <I
                src="/static/logotypes/3for2.png"
                srcSet="/static/logotypes/3for2.png 2x"
                alt="3for2"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/acando.png"
                srcSet="/static/logotypes/acando.png 2x"
                alt="acando"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/accenture.png"
                srcSet="/static/logotypes/accenture.png 2x"
                alt="accenture"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/agder.png"
                srcSet="/static/logotypes/agder.png 2x"
                alt="agder"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/akita.png"
                srcSet="/static/logotypes/akita.png 2x"
                alt="akita"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/aliander.png"
                srcSet="/static/logotypes/aliander.png 2x"
                alt="aliander"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/allego.png"
                srcSet="/static/logotypes/allego.png 2x"
                alt="allego"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/amrita.png"
                srcSet="/static/logotypes/amrita.png 2x"
                alt="amrita"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/aparito.png"
                srcSet="/static/logotypes/aparito.png 2x"
                alt="aparito"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/apgsga.png"
                srcSet="/static/logotypes/apgsga.png 2x"
                alt="apgsga"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/arundo.png"
                srcSet="/static/logotypes/arundo.png 2x"
                alt="arundo"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/biilabs.png"
                srcSet="/static/logotypes/biilabs.png 2x"
                alt="Biilabs"
              />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/bkk.png" srcSet="/static/logotypes/bkk.png 2x" alt="bkk" />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/blocklab.png"
                srcSet="/static/logotypes/blocklab.png 2x"
                alt="blocklab"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/bosch.png"
                srcSet="/static/logotypes/bosch.png 2x"
                alt="bosch"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/crayon.png"
                srcSet="/static/logotypes/crayon.png 2x"
                alt="crayon"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/dnvgl.png"
                srcSet="/static/logotypes/dnvgl.png 2x"
                alt="dnvgl"
              />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/dt.png" srcSet="/static/logotypes/dt.png 2x" alt="dt" />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/dnb.png" srcSet="/static/logotypes/dnb.png 2x" alt="DNB" />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/elering.png"
                srcSet="/static/logotypes/elering.png 2x"
                alt="elering"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/enexis.png"
                srcSet="/static/logotypes/enexis.png 2x"
                alt="enexis"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/engie.png"
                srcSet="/static/logotypes/engie.png 2x"
                alt="engie"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/entra.png"
                srcSet="/static/logotypes/entra.png 2x"
                alt="entra"
              />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/epm.png" srcSet="/static/logotypes/epm.png 2x" alt="epm" />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/ewe.png" srcSet="/static/logotypes/ewe.png 2x" alt="ewe" />
            </Li>
            <Li>
              <I src="/static/logotypes/ey.png" srcSet="/static/logotypes/ey.png 2x" alt="ey" />
            </Li>
            <Li>
              <I
                src="/static/logotypes/farmboek.png"
                srcSet="/static/logotypes/farmboek.png 2x"
                alt="farmboek"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/farmforce.png"
                srcSet="/static/logotypes/farmforce.png 2x"
                alt="farmforce"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/fourc.png"
                srcSet="/static/logotypes/fourc.png 2x"
                alt="fourc"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/frankfurt.png"
                srcSet="/static/logotypes/frankfurt.png 2x"
                alt="frankfurt"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/fujitsu.png"
                srcSet="/static/logotypes/fujitsu.png 2x"
                alt="fujitsu"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/gasunie.png"
                srcSet="/static/logotypes/gasunie.png 2x"
                alt="gasunie"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/grandcentrix.png"
                srcSet="/static/logotypes/grandcentrix.png 2x"
                alt="grandcentrix"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/grid.png"
                srcSet="/static/logotypes/grid.png 2x"
                alt="grid"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/handelshoyskolen.png"
                srcSet="/static/logotypes/handelshoyskolen.png 2x"
                alt="handelshoyskolen"
              />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/ice.png" srcSet="/static/logotypes/ice.png 2x" alt="ice" />
            </Li>{' '}
            {/* <Li>
          <I
            src="/static/logotypes/illaris.png"
            srcSet="/static/logotypes/illaris.png 2x"
            alt="Illaris LAB"
          />
        </Li>{' '} */}
            <Li>
              <I
                src="/static/logotypes/innoenergy.png"
                srcSet="/static/logotypes/innoenergy.png 2x"
                alt="innoenergy"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/intelzone.png"
                srcSet="/static/logotypes/intelzone.png 2x"
                alt="intelzone"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/ismb.png"
                srcSet="/static/logotypes/ismb.png 2x"
                alt="ismb"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/kpmg.png"
                srcSet="/static/logotypes/kpmg.png 2x"
                alt="kpmg"
              />
            </Li>{' '}
            {/* <Li>
          <I
            src="/static/logotypes/microsoft.png"
            srcSet="/static/logotypes/microsoft.png 2x"
            alt="Logo"
          />
        </Li>{' '} */}
            <Li>
              <I
                src="/static/logotypes/movimento.png"
                srcSet="/static/logotypes/movimento.png 2x"
                alt="movimento"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/multiconsult.png"
                srcSet="/static/logotypes/multiconsult.png 2x"
                alt="multiconsult"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/nordic.png"
                srcSet="/static/logotypes/nordic.png 2x"
                alt="nordic"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/nordic_semiconductor.png"
                srcSet="/static/logotypes/nordic_semiconductor.png 2x"
                alt="nordic semiconductor"
              />
            </Li>{' '}
            <Li>
              <I src="/static/logotypes/nte.png" srcSet="/static/logotypes/nte.png 2x" alt="nte" />
            </Li>
            <Li>
              <I
                src="/static/logotypes/ntnu.png"
                srcSet="/static/logotypes/ntnu.png 2x"
                alt="ntnu"
              />
            </Li>
            <Li>
              <I src="/static/logotypes/occ.png" srcSet="/static/logotypes/occ.png 2x" alt="occ" />
            </Li>
            <Li>
              <I
                src="/static/logotypes/orange.png"
                srcSet="/static/logotypes/orange.png 2x"
                alt="orange"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/oslo.png"
                srcSet="/static/logotypes/oslo.png 2x"
                alt="oslo"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/philips.png"
                srcSet="/static/logotypes/philips.png 2x"
                alt="philips"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/poyry.png"
                srcSet="/static/logotypes/poyry.png 2x"
                alt="poyry"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/RWTH.png"
                srcSet="/static/logotypes/RWTH.png 2x"
                alt="RWTH Aachen"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/samsung.png"
                srcSet="/static/logotypes/samsung.png 2x"
                alt="samsung"
              />
            </Li>
            {/* <Li>
          <I
            src="/static/logotypes/schneider.png"
            srcSet="/static/logotypes/schneider.png 2x"
            alt="Logo"
          />
        </Li> */}
            <Li>
              <I
                src="/static/logotypes/schuco.png"
                srcSet="/static/logotypes/schuco.png 2x"
                alt="schuco"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/south-east-water.png"
                srcSet="/static/logotypes/south-east-water.png 2x"
                alt="South East Water"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/sweco.png"
                srcSet="/static/logotypes/sweco.png 2x"
                alt="Sweco"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/tele2.png"
                srcSet="/static/logotypes/tele2.png 2x"
                alt="tele2"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/telstra.png"
                srcSet="/static/logotypes/telstra.png 2x"
                alt="telstra"
              />
            </Li>{' '}
            <Li>
              <I
                src="/static/logotypes/tine.png"
                srcSet="/static/logotypes/tine.png 2x"
                alt="tine"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/trianel.png"
                srcSet="/static/logotypes/trianel.png 2x"
                alt="trianel"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/trondheim.png"
                srcSet="/static/logotypes/trondheim.png 2x"
                alt="trondheim"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/tukio.png"
                srcSet="/static/logotypes/tukio.png 2x"
                alt="tukio"
              />
            </Li>
            <Li>
              <I src="/static/logotypes/tum.png" srcSet="/static/logotypes/tum.png 2x" alt="tum" />
            </Li>
            <Li>
              <I src="/static/logotypes/uio.png" srcSet="/static/logotypes/uio.png 2x" alt="uio" />
            </Li>
            <Li>
              <I
                src="/static/logotypes/undc.png"
                srcSet="/static/logotypes/undc.png 2x"
                alt="undc"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/USC_viterbi.png"
                srcSet="/static/logotypes/USC_viterbi.png 2x"
                alt="USC Viterbi"
              />
            </Li>
            <Li>
              <I
                src="/static/logotypes/yield.png"
                srcSet="/static/logotypes/yield.png 2x"
                alt="yield"
              />
            </Li>
          </Ul>
        </C>
      </S>
    );
  }
}

const S = styled.section`
  background-image: linear-gradient(-189deg, #eaf0f4 1%, #f3f8fa 95%);
  padding: 90px 0 70px;
  transform: skewY(-2deg);
  @media (max-width: 760px) {
    padding-bottom: 45px;
  }
`;
const C = styled.div`
  transform: skewY(2deg);
  width: 100%;
  max-width: 1440px;
  padding: 0 15px;
  margin-right: auto;
  margin-left: auto;
`;
const H = styled.p`
  font-size: 13px;
  font-weight: 800;
  letter-spacing: 0.84px;
  text-align: center;
  text-transform: uppercase;
  color: rgba(137, 156, 166, 1);

  @media (max-width: 760px) {
    margin-bottom: 40px;
  }
`;

const Ul = styled.ul`
  display: flex;
  justify-content: center;
  align-items: center;
  flex-wrap: wrap;
  margin: 40px auto;
  width: 80%;
  list-style: none;
  @media (max-width: 1120px) {
    justify-content: space-around;
  }
  @media (max-width: 760px) {
    flex-flow: row wrap;
    max-width: 370px;
    margin: 0 auto;
  }
`;
const Li = styled.li`
  margin: 0;
  padding: 0;
  border: 0;
  font-size: 100%;
  vertical-align: baseline;
  @media (max-width: 1120px) {
    &:not(:last-of-type) {
      margin-right: 0;
    }
  }
  @media (max-width: 760px) {
    margin-bottom: 40px;
  }
`;
const I = styled.img`
  max-height: 120px;
  max-width: 200px;
  padding: 10px 15px;
  @media (max-width: 760px) {
    max-width: 150px;
  }
`;
